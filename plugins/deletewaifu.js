...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command, text }) => {
    try {
      ...
      if (!args.length) {
        return msg.reply(`❇️ Debes especificar un personaje para eliminar.\n> Ejemplo » *${usedPrefix + command} Yuki Suou*`);
      }      
      ...
      if (!character) {
        return msg.reply(`❇️ No se ha encontrado ningún personaje con el nombre *${inputName}*\n> Puedes sugerirlo usando *${usedPrefix}suggest personaje ${inputName}*`);
      }      
      if (!user.characters.includes(character.id)) {
        return msg.reply(`❇️ *${character.name}* no está reclamado por ti.`);
      }
