...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command, text }) => {
    if (!text) return msg.reply('❇️ Por favor, ingresa el enlace de Mediafire.')
    try {
      if (!/^https?:\/\/(www\.)?mediafire\.com\/.+/i.test(text)) {
        return msg.reply('❇️ Por favor, ingresa un enlace válido de Mediafire.')
      }      
      const scraped = await mediafireDl(text)
      if (!scraped?.downloadLink) return msg.reply(`❇️ No se pudo obtener el archivo.`)      
