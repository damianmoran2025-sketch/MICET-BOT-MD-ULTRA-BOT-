...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command }) => {
    if (!args[0]) {
      return msg.reply('❇️ Por favor, ingresa un link de Google Drive.')
    }    
    const url = args[0]
    if (!url.match(/drive\.google\.com\/(file\/d\/|open\?id=|uc\?id=)/)) {
      return msg.reply('❇️ La URL no parece válida de Google Drive.')
    }    
    try {
      const result = await gdriveScraper(url)
      if (!result.status) {
        return msg.reply('❇️ No se pudo obtener el archivo. Intenta con otro enlace.')
      }      
