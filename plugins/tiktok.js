...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command }) => {
    ...
    try {
      ...
      if (!json.status) return msg.reply('❇️ No se encontró contenido válido en TikTok.')
      if (isUrl) {
...
𖣣ֶㅤ֯⌗ ✉️  ׄ ⬭ *Comentarios:* ${(stats?.comments || 0).toLocaleString()}
𖣣ֶㅤ֯⌗ ♣️ ׄ ⬭ *Vistas:* ${(stats?.views || stats?.plays || 0).toLocaleString()}
𖣣ֶㅤ֯⌗ 🔰  ׄ ⬭ *Compartidos:* ${(stats?.shares || 0).toLocaleString()}
𖣣ֶㅤ֯⌗ 💠  ׄ ⬭ *Fecha:* ${created_at || 'N/A'}`.trim()
        if (type === 'image') {
