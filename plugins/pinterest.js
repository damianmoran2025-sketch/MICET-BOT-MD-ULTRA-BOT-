...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command }) => {
    ...
    if (!text) {
      return msg.reply('❇️ Por favor, ingresa un término de búsqueda o un enlace de Pinterest.')
    }
    ...
    try {
      if (isPinterestUrl) {
        ...
        if (!medias.length) {
          return msg.reply(`✳️ No se pudieron obtener descargas válidas para *${text}*.`)
        }
